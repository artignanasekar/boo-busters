class Wave2 extends Phaser.Scene {
    ...
    preload() {
        ...
        if (document.getElementById('description')) {
            document.getElementById('description').innerHTML = 
                '<h2>Wave 2<br>W - move up // S - move down<br>SPACE - fire bullets</h2>';
        }
    }

    create() {
        this.add.rectangle(0, 0, this.game.config.width, this.game.config.height, 0x330000).setOrigin(0);
        ...
        for (let i = 0; i < 100; i++) {
            ...
            this.add.circle(x, y, size, 0xffaaaa, alpha);
        }
        ...
        this.ghostTimer = 0;
        this.ghostSpawnRate = 1300; // slightly faster
    ...
    spawnBigGhost() {
        ...
        ghost.setScale(1.5);
        ghost.setVelocityX(-220);
        ghost.setBounce(1);
        ghost.setCollideWorldBounds(true);
        ghost.setVelocityY(Phaser.Math.Between(-120, 120));
        ghost.lastFired = 0;
        ghost.fireRate = Phaser.Math.Between(700, 1800);
    }
    ...
    update(time, delta) {
        ...
        if (time > this.ghostTimer + this.ghostSpawnRate) {
            this.spawnGhost();
            if (Math.random() < 0.4) this.spawnBigGhost(); // more frequent
